...
4. **Resume.** On login, query the sheet for image_ids this curator has already
   rated and present only the remaining ones. Idempotent: a given
   (curator_id, image_id, repeat_index) is written at most once.
...
6. **Randomized, per-curator order.** Present the 60 in an order seeded by
   `hash(curator_id)` so it is reproducible but differs across curators
   (removes order/fatigue confounds on specific images).
...
8. **Intra-rater repeats.** Silently append a small number (config
   ...
   in that curator's queue, to measure within-rater consistency. So each curator
   sees 60 + N_REPEATS items. Repeats share the `image_id` but get a distinct
   `repeat_index`. Never reveal that repeats exist.

## Data model

### Curator list
A committed `curators.csv` (or a list in `st.secrets`) of allowed `curator_id`
values, shown as a **dropdown** at login (avoids free-text typos that fragment
data). No passwords — low-stakes, internal.

### Results sheet (Google Sheet, one row appended per rating)
Columns: `timestamp_iso`, `curator_id`, `image_id`, `repeat_index`,
`score` (float 0–1, blank if unsure), `unsure` (bool), `dwell_seconds`
(time image was on screen), `slider_touched` (bool), `queue_position`,
`app_version`.
...
2. **Curator identification.** Dropdown of `curator_id`s. On select, resume.
...
- **storage.py:** `append_result(row: dict)` and
  `completed(curator_id) -> set[(image_id, repeat_index)]`, implemented against
...
- **Idempotency:** filter the queue by `completed(curator_id)` on load; guard
  the append so refreshes/re-submits don't double-write.
...
7. `curators.csv` example with a few placeholder IDs.
8. `README.md`: local run; create Google service account + share Sheet; set
   Streamlit Cloud secrets; run resize script; deploy; export/analyze results.

## Out of scope / do NOT do

- No login/passwords beyond the curator dropdown.
- No admin UI.
